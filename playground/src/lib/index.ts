...
export const swagger_evi = (api_path: string[] | undefined | any) => {
     console.log(api_path);

     const resolvedBaseDir = api_path.find((p: any) => fs.existsSync(p)) || "src/module";
     ...
     for (const moduleName of modules) {
          ...
          const files = fs.readdirSync(modulePath).filter((file) => file.match(/\.(controller|module)\.(ts|js)$/));
